<?php
class dokusubscription_DokuPayment {
	...
	function __construct($configs = array()) {
		$dokupayment_enpoint = array(
			'sandbox'		=> array(
					...
					'registerupdate'	=> 'https://staging.doku.com/Suite/RecurUpdateCard',
					'registerdelete'	=> 'https://staging.doku.com/AccountBilling/DoDeleteSubscriptionSuite',
			),
			'live'			=> array(
					...
					'registerupdate'	=> 'https://pay.doku.com/Suite/RecurUpdateCard',
					'registerdelete'	=> 'https://pay.doku.com/AccountBilling/DoDeleteSubscriptionSuite',
			),
		);
	...
	public static function init($configs) {
        if (!self::$instance) {
            self::$instance = new dokusubscription_DokuPayment($configs);
        }
		...
		switch(strtolower($method)) {
			...
			default:
				...
				$PaymentStructure = array(
					...
					'EMAIL'					=> filter_var($user_input['email'], FILTER_VALIDATE_EMAIL),
			...
			case 'redirect-checking':
				$create_words = array(
					'mallid'				=> $this->mallid,
					'sharedkey'				=> $this->sharedkey,
					'amount'				=> (isset($params_input['subscription_amount']) ? sprintf("%.2f", floatval($params_input['subscription_amount'])) : '0.00'),
					'transaction_id'		=> (isset($params_input['subscription_transaction_id']) ? $params_input['subscription_transaction_id'] : 0),
					'transaction_words'		=> (isset($params_input['subscription_words']) ? $params_input['subscription_words'] : ''),
					'trans_status'			=> (isset($params_input['subscription_verify_status']) ? (string)$params_input['subscription_verify_status'] : ''), // 00
					'payment_channel'		=> (isset($params_input['subscription_transaction_channel']) ? $params_input['subscription_transaction_channel'] : ''),
					'session_id'			=> (isset($params_input['subscription_transaction_session']) ? $params_input['subscription_transaction_session'] : ''),
					'customer_id'			=> (isset($params_input['subscription_transaction_customer']) ? $params_input['subscription_transaction_customer'] : ''),
					'bill_number'			=> (isset($params_input['subscription_transaction_bill']) ? $params_input['subscription_transaction_bill'] : ''),
				);
				##
				$PaymentStructure = array(
					...
					'PAYMENTCHANNEL'		=> $create_words['payment_channel'],
					'SESSIONID'				=> $create_words['session_id'],
					'CUTOMERID'				=> $create_words['customer_id'],
					'BILLNUMBER'			=> $create_words['bill_number'],
				);
			break;
			case 'regupdate':
				$create_words = array(
					'mallid'				=> $this->mallid,
					'sharedkey'				=> $this->sharedkey,
					'chainmerchant'			=> (isset($params_input['subscription_chainmerchant']) ? $params_input['subscription_chainmerchant'] : $this->chainmerchant),
					'amount'				=> (isset($params_input['subscription_amount']) ? sprintf("%.2f", floatval($params_input['subscription_amount'])) : '0.00'),
					'transaction_id'		=> (isset($params_input['subscription_transaction_id']) ? $params_input['subscription_transaction_id'] : 0),
					'transaction_words'		=> (isset($params_input['subscription_words']) ? $params_input['subscription_words'] : ''),
					'transaction_datetime'	=> (isset($params_input['subscription_transaction_datetime']) ? $params_input['subscription_transaction_datetime'] : date('YmdHis')),
					'trans_status'			=> (isset($params_input['subscription_verify_status']) ? (string)$params_input['subscription_verify_status'] : ''), // 0000
					'payment_channel'		=> (isset($params_input['subscription_transaction_channel']) ? $params_input['subscription_transaction_channel'] : ''),
					'session_id'			=> (isset($params_input['subscription_transaction_session']) ? $params_input['subscription_transaction_session'] : ''),
					'customer_id'			=> (isset($params_input['subscription_transaction_customer']) ? $params_input['subscription_transaction_customer'] : ''),
					'bill_number'			=> (isset($params_input['subscription_transaction_bill']) ? $params_input['subscription_transaction_bill'] : ''),
				...
				##
				$PaymentStructure = array(
					'MALLID'				=> $create_words['mallid'],
					'CHAINMERCHANT'			=> $create_words['chainmerchant'],
					'TRANSIDMERCHANT'		=> $create_words['transaction_id'],
					'WORDS_STRING'			=> $this->create_words($method, $create_words),
					'WORDS'					=> sha1($this->create_words($method, $create_words)),
					'REQUESTDATETIME'		=> $create_words['transaction_datetime'],
					'SESSIONID'				=> $create_words['session_id'],
					'CUSTOMERID'			=> $create_words['customer_id'],
					'PAYMENTCHANNEL'		=> '17',
					'BILLNUMBER'			=> $create_words['bill_number'],
					'REGISTERAMOUNT'		=> $create_words['amount'],
				);
			break;
			case 'regsubscription':
				$create_words = array(
					...
					'chainmerchant'			=> $this->chainmerchant,
					'customer_id'			=> (isset($params_input['subscription_transaction_customer']) ? $params_input['subscription_transaction_customer'] : ''),
					'customer_name'			=> (isset($params_input['subscription_transaction_customer_name']) ? $params_input['subscription_transaction_customer_name'] : ''),
					'customer_email'		=> (isset($params_input['subscription_transaction_customer_email']) ? $params_input['subscription_transaction_customer_email'] : ''),
					'customer_phone'		=> (isset($params_input['subscription_transaction_customer_phone']) ? $params_input['subscription_transaction_customer_phone'] : ''),
					'customer_mobile'		=> (isset($params_input['subscription_transaction_customer_mobile']) ? $params_input['subscription_transaction_customer_mobile'] : ''),
					'bill_number'			=> (isset($params_input['subscription_transaction_bill']) ? $params_input['subscription_transaction_bill'] : ''),
				);
				##
				$PaymentStructure = array(
					'CUSTOMERID'			=> $create_words['customer_id'],
					'CUSTOMERNAME'			=> $create_words['customer_name'],
					'CUSTOMEREMAIL'			=> $create_words['customer_email'],
					'CUSTOMERHOMEPHONE'		=> $create_words['customer_phone'],
					'CUSTOMERMOBILEPHONE'	=> $create_words['customer_mobile'],
					'BILLNUMBER'			=> $create_words['bill_number'],
					'WORDS_STRING'			=> $this->create_words($method, $create_words),
					'WORDS'					=> sha1($this->create_words($method, $create_words)),
				);
			...
			case 'subscription':
				...
				if (isset($shipping_data)) {
					...
					# Recurring Billing
					//------------------------------------------------
					$PaymentStructure['CUSTOMERID'] = (isset($params_input['subscription']['CUSTOMERID']) ? $params_input['subscription']['CUSTOMERID'] : '');
					$PaymentStructure['BILLNUMBER'] = (isset($params_input['subscription']['BILLNUMBER']) ? $params_input['subscription']['BILLNUMBER'] : '');
					$PaymentStructure['BILLDETAIL'] = (isset($params_input['subscription']['BILLDETAIL']) ? $params_input['subscription']['BILLDETAIL'] : ''); // Details of payment
					$PaymentStructure['BILLTYPE'] = (isset($params_input['subscription']['BILLTYPE']) ? $params_input['subscription']['BILLTYPE'] : 'S'); // S = Shopping, I = Installment, D = Donation, P = Payment, B = Billing (Default is SALE)
					$PaymentStructure['STARTDATE'] = (isset($params_input['subscription']['STARTDATE']) ? $params_input['subscription']['STARTDATE'] : ''); // Format: YYYYMMDD
					$PaymentStructure['ENDDATE'] = (isset($params_input['subscription']['ENDDATE']) ? $params_input['subscription']['ENDDATE'] : 'NA'); // Format: YYYYMMDD
					$PaymentStructure['EXECUTETYPE'] = (isset($params_input['subscription']['EXECUTETYPE']) ? $params_input['subscription']['EXECUTETYPE'] : ''); # DAY / DATE / FULLDATE
					$PaymentStructure['EXECUTEDATE'] = (isset($params_input['subscription']['EXECUTEDATE']) ? $params_input['subscription']['EXECUTEDATE'] : ''); # If EXECUTETYPE = DAY (SUN / MON / TUE / WED / THU / FRI / SAT), If EXECUTETYPE = DATE (1 / 2 / 3 / ... / 28), If EXECUTETYPE = FULLDATE (Use format: YYYYMMDD)
					$PaymentStructure['EXECUTEMONTH'] = (isset($params_input['subscription']['EXECUTEMONTH']) ? $params_input['subscription']['EXECUTEMONTH'] : ''); # Please select one (JAN / FEB / MAR / APR / MAY / JUN / JUL / AUG / SEP / OCT / NOV / DEC)
					$PaymentStructure['FLATSTATUS'] = (isset($params_input['subscription']['FLATSTATUS']) ? $params_input['subscription']['FLATSTATUS'] : 'TRUE'); // Dinamyc AMOUNT = FALSE, Static (Fix) AMOUNT = TRUE
					# AMOUNT
					$PaymentStructure['REGISTERAMOUNT'] = (isset($params_input['subscription']['REGISTERAMOUNT']) ? $params_input['subscription']['REGISTERAMOUNT'] : '');
				}
			break;
			//------------------------ Recurring-Notify ------------------------
			case 'notify':
			case 'notify-checking':
				$create_words = array(
					...
					'sharedkey'				=> $this->sharedkey,
					'transaction_id'		=> (isset($params_input['subscription_transaction_id']) ? $params_input['subscription_transaction_id'] : 0),
					'result_msg'			=> (isset($params_input['subscription_result_msg']) ? $params_input['subscription_result_msg'] : ''), // SUCCESS
					'verify_status'			=> (isset($params_input['subscription_verify_status']) ? $params_input['subscription_verify_status'] : ''), // SUCCESS
					'currency'				=> (isset($params_input['subscription_transaction_currency']) ? $params_input['subscription_transaction_currency'] : 'ID'),
				);
				$create_words['currency'] = $this->get_countrycode($this->isofile, $create_words['currency']);
				##
				$PaymentStructure = array(
					'MALLID'				=> $this->mallid,
					'CHAINMERCHANT'			=> $this->chainmerchant,
			...
			case 'notify-recurring':
				$create_words = array(
					...
					'transaction_id'		=> (isset($params_input['subscription_transaction_id']) ? $params_input['subscription_transaction_id'] : 0),
					'session_id'			=> (isset($params_input['subscription_transaction_session']) ? $params_input['subscription_transaction_session'] : ''),
					'transaction_customer'	=> (isset($params_input['subscription_transaction_customer']) ? $params_input['subscription_transaction_customer'] : ''),
					'transaction_token'		=> (isset($params_input['subscription_transaction_token']) ? $params_input['subscription_transaction_token'] : ''),
					'result_msg'			=> (isset($params_input['subscription_result_msg']) ? $params_input['subscription_result_msg'] : ''), // SUCCESS
					'verify_status'			=> (isset($params_input['subscription_verify_status']) ? $params_input['subscription_verify_status'] : ''), // SUCCESS
					'currency'				=> (isset($params_input['subscription_transaction_currency']) ? $params_input['subscription_transaction_currency'] : 'ID'),
				);
				...
				$PaymentStructure = array(
					'MALLID'				=> $this->mallid,
					'CHAINMERCHANT'			=> $this->chainmerchant,
					'TRANSIDMERCHANT'		=> $create_words['subscription_transaction_id'],
					'SESSIONID'				=> $create_words['session_id'],
					'WORDS_STRING'			=> $this->create_words($method, $create_words),
					'WORDS'					=> sha1($this->create_words($method, $create_words)),
					'RESULTMSG'				=> $create_words['result_msg'],
			...
			break;
			case 'notify-subscription-checking':
			case 'notify-recurring-checking':
				$create_words = array(
					'amount'				=> (isset($params_input['subscription_amount']) ? sprintf("%.2f", floatval($params_input['subscription_amount'])) : '0.00'),
					'mallid'				=> $this->mallid,
					'sharedkey'				=> $this->sharedkey,
				...
				$create_words['currency'] = $this->get_countrycode($this->isofile, $create_words['currency']);
				$create_words['currency_purchase'] = $create_words['currency'];
				##
				$PaymentStructure = array(
					'MALLID'				=> $this->mallid,
					'CHAINMERCHANT'			=> $this->chainmerchant,
			...
			case 'subscription-notify':
				$create_words = array(
					...
					'chainmerchant'			=> (isset($params_input['subscription_chainmerchant']) ? $params_input['subscription_chainmerchant'] : $this->chainmerchant),
					'transaction_id'		=> (isset($params_input['subscription_transaction_id']) ? $params_input['subscription_transaction_id'] : 0),
					'transaction_bill'		=> (isset($params_input['subscription_transaction_bill']) ? $params_input['subscription_transaction_bill'] : ''),
					'transaction_customer'	=> (isset($params_input['subscription_transaction_customer']) ? $params_input['subscription_transaction_customer'] : ''),
					'transaction_card'		=> (isset($params_input['subscription_transaction_card']) ? $params_input['subscription_transaction_card'] : ''),
					'transaction_status'	=> (isset($params_input['subscription_transaction_status']) ? $params_input['subscription_transaction_status'] : ''), // SUCCESS
					'transaction_error'		=> (isset($params_input['subscription_transaction_error']) ? $params_input['subscription_transaction_error'] : ''),
					'transaction_message'	=> (isset($params_input['subscription_transaction_message']) ? $params_input['subscription_transaction_message'] : ''),
					'transaction_type'		=> (isset($params_input['subscription_transaction_type']) ? $params_input['subscription_transaction_type'] : ''), // G: Notify Registration T: Notify Update
					'currency'				=> (isset($params_input['subscription_transaction_currency']) ? $params_input['subscription_transaction_currency'] : 'ID'),
				);
				$create_words['currency'] = $this->get_countrycode($this->isofile, $create_words['currency']);
				##
				$PaymentStructure = array(
					'MALLID'				=> $this->mallid,
					'CHAINMERCHANT'			=> $create_words['chainmerchant'],
					'CUSTOMERID'			=> $create_words['transaction_customer'],
					'BILLNUMBER'			=> $create_words['transaction_bill'],
					'TRANSIDMERCHANT'		=> $create_words['transaction_id'],
					'CARDNUMBER'			=> $create_words['transaction_card'],
					'STATUS'				=> $create_words['transaction_status'],
					'ERRORCODE'				=> $create_words['transaction_error'],
					'MESSAGE'				=> $create_words['transaction_message'],
					'WORDS_STRING'			=> $this->create_words($method, $create_words),
					'WORDS'					=> sha1($this->create_words($method, $create_words)),
					'STATUSTYPE'			=> $create_words['transaction_type'],
				);
			break;
			...
			case 'subscription-process':
				$create_words = array(
					...
					'transaction_type'		=> (isset($params_input['subscription_transaction_type']) ? $params_input['subscription_transaction_type'] : ''), // G: Notify Registration T: Notify Update
					'transaction_token'		=> (isset($params_input['subscription_transaction_token']) ? $params_input['subscription_transaction_token'] : ''),
					'transaction_code'		=> (isset($params_input['transaction_code']) ? $params_input['transaction_code'] : ''),
					'currency'				=> (isset($params_input['transaction_currency']) ? $params_input['transaction_currency'] : '360'),
					'transaction_approval'	=> (isset($params_input['transaction_approval']) ? $params_input['transaction_approval'] : ''),
					'transaction_resultmsg'	=> (isset($params_input['subscription_result_msg']) ? $params_input['subscription_result_msg'] : ''),
					'transaction_bank'		=> (isset($params_input['transaction_bank']) ? $params_input['transaction_bank'] : ''),
					'verify_id'				=> (isset($params_input['subscription_verify_id']) ? $params_input['subscription_verify_id'] : ''),
					'verify_score'			=> (isset($params_input['subscription_verify_score']) ? $params_input['subscription_verify_score'] : ''),
					'verify_status'			=> (isset($params_input['subscription_verify_status']) ? $params_input['subscription_verify_status'] : ''),
					'transaction_session'	=> (isset($params_input['subscription_transaction_session']) ? $params_input['subscription_transaction_session'] : ''),
					'transaction_datetime'	=> (isset($params_input['transaction_datetime']) ? $params_input['transaction_datetime'] : date('YmdHis')),
				...
				$PaymentStructure = array(
					...
					'CHAINMERCHANT'			=> $create_words['chainmerchant'],
					'CUSTOMERID'			=> $create_words['transaction_customer'],
					'TOKENID'				=> $create_words['transaction_token'],
					'CARDNUMBER'			=> $create_words['transaction_card'],
					'BILLNUMBER'			=> $create_words['transaction_bill'],
					'TRANSIDMERCHANT'		=> $create_words['transaction_id'],
					'AMOUNT'				=> $create_words['amount'],
					'CURRENCY'				=> $create_words['currency'],
					'RESPONSECODE'			=> $create_words['transaction_code'],
					'APPROVALCODE'			=> $create_words['transaction_approval'],
					'RESULTMSG'				=> $create_words['transaction_resultmsg'],
					'BANK'					=> $create_words['transaction_bank'],
					'VERIFYID'				=> $create_words['verify_id'],
					'VERIFYSCORE'			=> $create_words['verify_score'],
					'VERIFYSTATUS'			=> $create_words['verify_status'],
					'SESSIONID'				=> $create_words['transaction_session'],
					'PAYMENTDATETIME'		=> $create_words['transaction_datetime'],
					'WORDS_STRING'			=> $this->create_words($method, $create_words),
					'WORDS'					=> sha1($this->create_words($method, $create_words)),
				);
			break;
		...
		switch(strtolower($method)) {
			...
			case 'redirect-checking':
				$create_words = array(
					...
					'sharedkey'			=> (isset($params_input['sharedkey']) ? $params_input['sharedkey'] : $this->sharedkey),
					'transaction_id'	=> (isset($params_input['transaction_id']) ? $params_input['transaction_id'] : 0),
					'customer_id'		=> (isset($params_input['customer_id']) ? $params_input['customer_id'] : ''),
					'bill_number'		=> (isset($params_input['bill_number']) ? (string)$params_input['bill_number'] : ''),
				);
			break;
			case 'regupdate':
				$create_words = array(
					'mallid'			=> (isset($params_input['mallid']) ? $params_input['mallid'] : $this->mallid),
					'chainmerchant'		=> (isset($params_input['chainmerchant']) ? $params_input['chainmerchant'] : $this->chainmerchant),
					'bill_number'		=> (isset($params_input['bill_number']) ? (string)$params_input['bill_number'] : ''),
					'customer_id'		=> (isset($params_input['customer_id']) ? $params_input['customer_id'] : ''),
					'sharedkey'			=> (isset($params_input['sharedkey']) ? $params_input['sharedkey'] : $this->sharedkey),
				);
			...
			case 'notify-recurring':
				$create_words = array(
					...
					'mallid'			=> (isset($params_input['mallid']) ? $params_input['mallid'] : $this->mallid),
					'sharedkey'			=> (isset($params_input['sharedkey']) ? $params_input['sharedkey'] : $this->sharedkey),
					'customerid'		=> (isset($params_input['transaction_customer']) ? $params_input['transaction_customer'] : ''),
					'tokenid'			=> (isset($params_input['transaction_token']) ? $params_input['transaction_token'] : ''),
					'result_msg'		=> (isset($params_input['result_msg']) ? $params_input['result_msg'] : ''), // SUCCESS
					'verify_status'		=> (isset($params_input['verify_status']) ? $params_input['verify_status'] : ''), // REVIEW
				);
				//(transidmerchant+mallid+sharedkey+customerid+tokenid+resultmsg+verifystatus)
			break;
			case 'notify-subscription-checking':
			case 'notify-recurring-checking':
				$create_words = array(
			...
			case 'subscription-notify':
				$create_words = array(
					'mallid'				=> (isset($params_input['mallid']) ? $params_input['mallid'] : $this->mallid),
					'chainmerchant'			=> (isset($params_input['chainmerchant']) ? $params_input['chainmerchant'] : $this->chainmerchant),
					'transaction_bill'		=> (isset($params_input['transaction_bill']) ? $params_input['transaction_bill'] : 0),
					'transaction_customer'	=> (isset($params_input['transaction_customer']) ? $params_input['transaction_customer'] : ''),
					'transaction_status'	=> (isset($params_input['transaction_status']) ? $params_input['transaction_status'] : ''), // SUCCESS
					'sharedkey'				=> (isset($params_input['sharedkey']) ? $params_input['sharedkey'] : $this->sharedkey),
				);
			break;
			case 'subscription-update':
			...
			case 'subscription-process':
				$create_words = array(
					'transaction_id'		=> (isset($params_input['transaction_id']) ? $params_input['transaction_id'] : ''),
					'mallid'				=> (isset($params_input['mallid']) ? $params_input['mallid'] : $this->mallid),
					'sharedkey'				=> (isset($params_input['sharedkey']) ? $params_input['sharedkey'] : $this->sharedkey),
					'transaction_customer'	=> (isset($params_input['transaction_customer']) ? $params_input['transaction_customer'] : ''),
					'transaction_token'		=> (isset($params_input['transaction_token']) ? $params_input['transaction_token'] : ''),
					'transaction_bill'		=> (isset($params_input['transaction_bill']) ? $params_input['transaction_bill'] : 0),
					'transaction_resultmsg'	=> (isset($params_input['transaction_resultmsg']) ? $params_input['transaction_resultmsg'] : ''),
					'verify_status'			=> (isset($params_input['verify_status']) ? $params_input['verify_status'] : ''),
				);
			break;
		}
	...
	// Create Dateobject
	function get_datetime_object($timezone = 'Asia/Bangkok') {
